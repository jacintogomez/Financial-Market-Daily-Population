...
fundamentals_url='https://financialmodelingprep.com/api/v3/quotes/index?apikey='
...
def ai_summary(request,stock_symbol):
    llm=ChatOpenAI()
    url=f'https://financialmodelingprep.com/api/v3/stock_news?tickers={stock_symbol}&apikey={fmp_key}'
    news_stories=requests.get(url).json()
    formatted_data=json.dumps(news_stories,indent=4)
    ...
    task=f'Please give a general summary of the relevant financial news stories for {stock_symbol}\n{formatted_data}'
